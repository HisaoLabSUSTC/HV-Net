/*
	HV_PolarC.cpp		Mex C++ file for hypervolume approximation in polar coordinate system
	
	Usage: HV_PolarC(points, reference point, samples)
...
void ConstructLattice(int m, int w, double **weights, double *jacobians)
{
	...
	double eps = 1e-5;
	int p = (m-1)*2+1;		// the dimension of angle is 1 smaller than m
	bool flag = true;
	while(flag)
	{
		flag = false;
	...
	int *a = new int[m-1];	// the dimension of angle is 1 smaller than m
	a[0] = 1;
	for(int i=1; i<m-1; i++)
	{
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	int m, n, w;
	double *points, *reference, Length;
	bool flag;
	double *hv;
